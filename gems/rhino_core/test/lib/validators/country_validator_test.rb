...
class CountryValidatorTest < ActiveSupport::TestCase
  ...
  test "validates alpha2 country with allow_blank" do
    class CountryAlpha2Blank < Country
      validates :country, country: { allow_blank: true }
    end

    assert_predicate CountryAlpha2Blank.new, :valid?
    assert_predicate CountryAlpha2Blank.new(country: "ZZ"), :invalid?
    assert_predicate CountryAlpha2Blank.new(country: "CA"), :valid?
  end
  ...
  test "validates alpha3 country with allow_blank" do
    class CountryAlpha3Blank < Country
      validates :country, country: { alpha3: true, allow_blank: true }
    end

    assert_predicate CountryAlpha3Blank.new, :valid?
    assert_predicate CountryAlpha3Blank.new(country: "ZZZ"), :invalid?
    assert_predicate CountryAlpha3Blank.new(country: "CAN"), :valid?
    assert_predicate CountryAlpha3Blank.new(country: "CA"), :invalid?
  end
end
